...
def query_maven(sha1, filename):
    ...
    if found_items == 0:
        ...
        return {'sha1':sha1,'filename':filename,'id':'UNKNOWN','ts':'UNKNOWN','tags':'UNKNOWN'}
    if found_items == 1:
        package_id = data['response']['docs'][0]['id']
        timestamp = datetime.datetime.fromtimestamp(int(data['response']['docs'][0]['timestamp'])/1000).strftime('%Y.%m.%d %H:%M:%S')
        tags = data['response']['docs'][0]['tags']
        print(f'{sha1}\t{filename}\t\t{package_id}')
        return {'sha1':sha1,'filename':filename,'id':package_id,'ts':timestamp, 'tags':tags}
    if found_items > 1:
        ...
        return {'sha1':sha1,'filename':filename,'id':packages,'ts':timestamp,'tags':tags}
    else:
...
def parse_sha1file_and_annotate(sha1_filepath):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        future_to_hash = {executor.submit(query_maven, pair[0] , pair[1]): pair for pair in hash_file_pairs}
